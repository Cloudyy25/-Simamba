...
<body class="flex flex-col lg:flex-row min-h-screen">
    <div class="flex flex-col lg:grid lg:grid-cols-[250px_1fr] h-screen">
        ...
        <div class="flex flex-col overflow-y-auto">
            <header
                class="flex justify-between items-center bg-gradient-to-r from-yellow-400 to-orange-400 px-6 py-4 mt-auto text-white">
                <!-- Hamburger Icon -->
                    ...
                    <div class="flex flex-wrap gap-6 items-center justify-between">
                        ...
                        <div class="flex flex-wrap gap-4 items-center w-full sm:w-auto mb-4 ">
                            ...
                            <form action="{{ route('kegiatan.index') }}" method="GET"
                                class="flex flex-wrap gap-4 items-center w-full sm:w-auto mb-4">
                                ...
                                <select name="team"
                                    ...
                                    @foreach ($teams as $team)
                                        <option value="{{ $team }}"
                                            {{ request('team') == $team ? 'selected' : '' }}>{{ $team }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <!-- Tombol Filter -->
                                <button type="submit"
                                    class="py-2 px-4 bg-orange-500 text-white text-lg rounded-md w-full sm:w-48">Filter</button>
                            </form>
                        ...
                        <div class="flex flex-wrap gap-6 items-center justify-between w-full">
                            ...
                            <div class="flex gap-4 flex-wrap w-full sm:w-auto mb-4">
                                <!-- Tombol Excel -->
                                <a href="{{ route('kegiatan.download', ['format' => 'excel']) }}"
                                    class="py-2 px-4 bg-green-500 text-white text-lg rounded-md w-full sm:w-40">Excel</a>

                                <!-- Tombol CSV -->
                                <a href="{{ route('kegiatan.download', ['format' => 'csv']) }}"
                                    class="py-2 px-4 bg-green-500 text-white text-lg rounded-md w-full sm:w-40">CSV</a>

                                <!-- Tombol Print -->
                                <a href="{{ route('kegiatan.print') }}" target="_blank"
                                    class="py-2 px-4 bg-green-500 text-white text-lg rounded-md w-full sm:w-40">Print</a>
                            </div>
                            ...
                            <div class="flex gap-2 ml-auto items-center">
                                ...
                                <input type="text"
                                    class="py-2 px-4 text-lg border-2 border-orange-500 rounded-md w-48"
                                ...
                                @forelse ($kegiatan as $item)
                                    <tr class="text-center border-b border-orange-500">
                                        ...
                                        <td class="p-2 border border-orange-500">
                                            {{ \Carbon\Carbon::parse($item->mulai)->format('j F Y') }}</td>
                                        <td class="p-2 border border-orange-500">
                                            {{ \Carbon\Carbon::parse($item->berakhir)->format('j F Y') }}</td>
    ...
    <script>
        ...
        function handleSearch(event) {
            ...
            if (query === '') {
                // Jika pencarian dihapus, kembalikan ke format asli
                paginationText.textContent =
                    `Showing {{ $kegiatan->firstItem() }} to {{ $kegiatan->lastItem() }} of {{ $kegiatan->total() }} entries`;
            } else {
                // Jika pencarian aktif, tampilkan format pencarian
                paginationText.textContent =
