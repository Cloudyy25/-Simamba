...
class EvaluasiController extends Controller
{
    public function evaluasiKegiatan(Request $request)
    {
        ...
        // Ambil data kegiatan yang sudah dievaluasi dan belum dievaluasi berdasarkan tim PJ
        $sudahDievaluasi = Kegiatan::where('status', 'tidak aktif')
            ->whereHas('evaluasis')
            ->when($user->id_jabatan == 2, function ($query) use ($user) {
                $query->where('id_tim', $user->id_tim); // Filter kegiatan berdasarkan tim PJ
            })
            ->get();
