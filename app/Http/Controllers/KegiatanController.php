...
class KegiatanController extends Controller
{
    ...
    public function daftarKegiatan(Request $request)
    {
        ...
        if ($month) {
            $query->where(function ($q) use ($month) {
                $q->whereMonth('tanggal_mulai', $month)
                    ->orWhereMonth('tanggal_berakhir', $month);
            });
        }
        ...
        // Menambahkan filter berdasarkan tim untuk Penanggung Jawab (PJ)
        $user = Auth::user();
        if ($user->id_jabatan == 2) { // Jika user adalah Penanggung Jawab (PJ)
            $query->where('id_tim', $user->id_tim);
        }

        // Urutkan berdasarkan kegiatan yang paling baru ditambahkan
    ...
    {
        // Ambil data dari tabel `tims`
        $tims = Tim::all();

        // Kirim data ke view `pj.tambahkegiatan`
        return view('pj.tambahkegiatan', compact('tims'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            ...
            'realisasi' => 'required|numeric|min:0|lte:target',
        ...
        // Cari tim berdasarkan id_tim milik user
        $tim = Tim::where('id_tim', $user->id_tim)->first();

        // Simpan data kegiatan ke dalam database dengan id_tim yang sesuai
        Kegiatan::create([
            'nama_kegiatan' => $request->nama_kegiatan,
            'id_tim' => $tim->id_tim, // ID tim yang ditemukan
    ...
    public function update(Request $request, $id_kegiatan)
    {
        ...
        // Menggunakan id_tim yang ada sebelumnya (tidak diubah)
        $timId = $kegiatan->id_tim;

        // Update data kegiatan dengan data yang sudah divalidasi
        $kegiatan->update(array_merge($validatedData, [
            'id_tim' => $timId,  // Tetap mempertahankan id_tim yang ada sebelumnya
            'tanggal_mulai' => $validatedData['mulai'], // Update tanggal_mulai
            'tanggal_berakhir' => $validatedData['berakhir'], // Update tanggal_berakhir
        ]));
